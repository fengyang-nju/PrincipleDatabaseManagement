...
int RBFtest_private_2b(RecordBasedFileManager *rbfm) {
	...
	// 1. Create File - RBFM
	// 2. Open File
	// 3. insertTuple() - a big size tuple so that two tuples cannot fit in a page.
	// 4. Close File
	// 5. Destroy File
	cout << endl << "***** In RBF Test Case Private 2b *****" << endl;
	...
	prepareLargeRecord(recordDescriptor.size(), nullsIndicator, 2060, record, &recordSize);
	...
	prepareLargeRecord(recordDescriptor.size(), nullsIndicator, 2061, record, &recordSize);
