...
int RBFTest_private_2(RecordBasedFileManager *rbfm) {
    ...
    // Insert 100 records into the file
	for (int i = 0; i < numRecords; i++) {
		memset(record, 0, 3000);
		prepareLargeRecord(recordDescriptor.size(), nullsIndicator, 2060+i, record, &recordSize);
    ...
    // One more insertion
	memset(record, 0, 3000);
	prepareLargeRecord(recordDescriptor.size(), nullsIndicator, 2160, record, &recordSize);
	...
	if (headerPageExists) {
		cout<<"Get into the header page exists part!"<<endl;
	    ...
	    // Each page can only contain one record. So, readPageCountDiff should be greater than 100
	    // since the system needs to go through all pages from the beginning.
	    if (readPageCountDiff < 100) {
	    	cout << "The implementation regarding insertRecord() is not correct." << endl;
	    	cout << "***** [FAIL] Test Case Private 2 Failed! *****" << endl;
		    rc = rbfm->closeFile(fileHandle);
		    rc = rbfm->destroyFile(fileName);
